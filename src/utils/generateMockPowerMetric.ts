function generateMockPowerMetric(index: number): PowerMetric {
  // Constants
  ...
  const voltageV2n = roundTo(
    NOMINAL_VOLTAGE * (1 + randomInRange(-0.05, 0.05)),
    1
  );
  ...
  const activePowerW1 = roundTo(
    voltageV1n * currentA1 * randomInRange(0.8, 1),
    0
  );
  const activePowerW2 = roundTo(
    voltageV2n * currentA2 * randomInRange(0.8, 1),
    0
  );
  const activePowerW3 = roundTo(
    voltageV3n * currentA3 * randomInRange(0.8, 1),
    0
  );
...
export { generateMockPowerMetric };